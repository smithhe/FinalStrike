...
- The `fixtures/sample-app` server (integration target) runs via `python -m sample_app.server 8080` from inside `fixtures/sample-app/` and serves the API (`GET /health`, `GET/POST /api/tasks`) plus static frontend on the same port. The tasks page is at `/tasks/` (title "Sample App - Tasks").

...
def test_vision_probe_classifies_image_input_errors(monkeypatch) -> None:
    class _FakeProvider:
        @classmethod
        def from_context(cls, *args, **kwargs):  # type: ignore[no-untyped-def]
            del args, kwargs
        ...
        def chat_completion_multimodal(self, *args, **kwargs):  # type: ignore[no-untyped-def]
            del args, kwargs
            raise LLMProviderError("model does not support image input")
    ...
    status = assess_computer_use_vision(type("Path", (), {})())
    assert status.ready is False
    assert "does not accept image input" in status.detail
